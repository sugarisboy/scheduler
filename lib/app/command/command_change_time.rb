...
class CommandChangeTime < Command
  ...
  def execute
    ...
    list = @scheduler_service.lectures_by_time(old_week_day, old_num_lecture)
    lecture = @question_select_lecture.ask(list)

    puts 'Выберем новую дату и время:'
    new_week_day = @question_week_day.ask
    new_num_lecture = @question_num_lecture.ask

    @updater_service.update_time(lecture, new_week_day, new_num_lecture)
  end
end
